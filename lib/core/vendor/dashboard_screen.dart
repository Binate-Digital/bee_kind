...
>>>>>>> Stashed changes
  ...
  void _showDailySales(BuildContext context, SalesData weekData) {
    ...
    if (dailySales.isEmpty) {
      // No daily data available, show a message
      showModalBottomSheet(
        ...
        builder: (_) {
          return Padding(
            ...
            child: Center(
              child: CustomText(
                text: "No daily sales data available for this period",
                fontSize: 14.sp,
    ...
    // Convert DailySales to DailySalesData for chart
    final dailyData = dailySales.map((ds) {
      final date = DateTime.tryParse(ds.date ?? '');
      final label = date != null
          ? "${DateFormat('E').format(date)}\n${DateFormat('MMM d').format(date)}"
          : ds.date ?? 'Unknown';
      return DailySalesData(label: label, sales: (ds.amount ?? 0).toDouble());
    }).toList();
      ...
      builder: (_) {
        return Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                "Daily Sales for ${weekData.label}",
...
=======
                    Obx(() {
                      ...
                      // Convert WeeklySales to SalesData for chart
                      final chartData = weeklySales.map((ws) {
                        return SalesData(
                          label: "Week ${ws.week ?? 0}",
                          sales: (ws.amount ?? 0).toDouble(),
                        );
                      }).toList();
                      ...
                      if (chartData.isEmpty) {
                        return Center(
                          child: CustomText(
                            text: "No weekly sales data available",
                            fontSize: 14.sp,
                            fontColor: AppColors.blackColor,
                          ),
                        );
                      ...
                      return SfCartesianChart(
                        ...
                        primaryYAxis: NumericAxis(
                          ...
                          majorGridLines: const MajorGridLines(width: 0.4),
                          axisLine: const AxisLine(width: 1),
                          maximum: getChartMaxY(chartData),
                        ),
                        tooltipBehavior: TooltipBehavior(enable: true),
