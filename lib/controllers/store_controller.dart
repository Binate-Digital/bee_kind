...
class StoreController extends GetxController {
  ...
  /// Fetch completed and cancelled orders for order history
  Future<void> fetchCompletedAndCancelledOrders() async {
    try {
      isFetchingOrders.value = true;
      ...
      final List<order.OrderData> mergedList = [];
  ...
  void selectAddress(int index) async {
    ...
    // Save selected address ID for persistence
    prefs.setString("selectedAddressId", selected?.id ?? "");
    // Force UI update
    update();
    if (selected?.id != null) {
    ...
    // Update map pin to show selected address location
    final baseController = Get.find<BaseViewController>();
    baseController.updateMapToSelectedAddress();
  }
  ...
  Future<void> fetchUserAddresses() async {
    try {
      ...
      if (data["status"] == true && data["data"] != null) {
        ...
        // First, try to load previously selected address from prefs
        final savedAddressId = prefs.getString("selectedAddressId");
        if (savedAddressId != null && savedAddressId.isNotEmpty) {
          final savedIndex = list.indexWhere((a) => a.id == savedAddressId);
          if (savedIndex != -1) {
            selectAddress(savedIndex);
          } else {
            // Saved address not found, fall back to default
        ...
        // Update map to show selected address location after addresses are loaded
        final baseController = Get.find<BaseViewController>();
        baseController.updateMapToSelectedAddress();
      } else {
  ...
  Future<bool> addReview({
    ...
    try {
      final response = await network.postRequest(
        endPoint: NetworkStrings.addReview,
        data: {
          "orderId": orderId,
          "productId": productId,
          "rating": rating,
      ...
      if (data["status"] == true) {
        AppDialogs.showToast("Review submitted successfully");
        return true;
      } else {
        AppDialogs.showToast(data["message"] ?? "Failed to submit review");
        return false;
      }
    } catch (e) {
      log("addReview Exception: $e");
      AppDialogs.showToast("Error submitting review");
      return false;
    }
  }
  ...
  Future<void> fetchCurrentTabOrders(BuildContext context) async {
    try {
      ...
      final results = await Future.wait([
        ...
        network.getRequest(
          endPoint: NetworkStrings.vendorGetOrders,
          queryParameters: {'status': 'ready-for-pickup'},
          isHeaderRequire: true,
          isToast: false,
        ),
        network.getRequest(
          endPoint: NetworkStrings.vendorGetOrders,
          queryParameters: {'status': 'dispatched'},
          isHeaderRequire: true,
          isToast: false,
        ),
      ]);
      ...
      final List<vendorOrder.VendorOrder> mergedList = [];

      for (var response in results) {
        if (response != null && response.data != null) {
          final model = vendorOrder.VendorOrdersResponseModel.fromJson(
            response.data,
          );
          if (model.data != null) {
            mergedList.addAll(model.data!);
      ...
      vendorOrders.value = mergedList;
      log(
        "Fetched ${mergedList.length} active orders (Accepted/Ready/Dispatched)",
      );
    } catch (e) {
      isLoading.value = false;
      log("fetchCurrentTabOrders Exception: $e");
      AppDialogs.showToast("Error fetching current orders");
    }
  }
    ...
    try {
      ...
      if (data["status"] == true && data["data"] != null) {
        try {
          ...
          Map<String, dynamic> orderData = responseData;

          // Check for nested order object
          if (responseData.containsKey('order') &&
              responseData['order'] is Map) {
            log('Found nested "order" key, using it.');
            orderData = responseData['order'];
            log('Nested Data Keys: ${orderData.keys.toList()}');
          }

          // If user data is separate, attach it to the order data
          if (responseData.containsKey('user') && responseData['user'] is Map) {
            log('Found separate user data, attaching to order.');
            orderData['user'] = responseData['user'];
          }
          ...
          // Verify
          log('Refined Parsed Order ID: ${selectedVendorOrder.value?.sId}');
          log('Refined Parsed User: ${selectedVendorOrder.value?.user?.name}');
          log(
            'Refined Parsed User FirstName: ${selectedVendorOrder.value?.user?.firstName}',
          );
          log(
